...
package object lists {
  ...
  /**
    ...
    * [[StackOverflowError]] for large lists (we say it's not stack safe). Convince yourself that this is the ase, and
    * then write another general list-recursion function, foldLeft, that is tail-recursive, using techniques we
    * discussed in the previous chapter. Here is the signature
    */
  ...
  /**
    * Exercise 3.12 Write a function that returns the reverse of a list. See if you can write it using a fold.
    */
  def reverse[A](as: List[A]): List[A] = foldLeft(as, List[A]())((acc, elem) => elem :: acc)

  /**
    * Exercise 3.13 Can you write [[foldLeft]] in terms of [[foldRight()]]? How about the other way around? Implementing
    * foldrRight via foldLeft is useful because it lets us implement foldRight tail-recursively, which means it works
  ...
  /**
    * Exercise 3.15 Write a function that concatenates a list of lists info a single list. Its runtime should be linear
    * in the total length of all lists. Try to use functions we have already defined
    */
  def concatAll[A](lists: List[List[A]]): List[A] = {
    foldRightViaFoldLeft(lists, List[A]())((list, acc) => foldRightViaFoldLeft(list, acc)((elem, acc2) => elem :: acc2))
  }
  ...
  /**
    ...
    *
    * @note
    *   the axis is the as param, the final list is only guaranteed to have values based on its values
    */
  ...
  /**
    ...
    *   - windowSize
    * @tparam A
    *   - type param of target list
    * @return
    *   list of all subsequences of target list
    */
